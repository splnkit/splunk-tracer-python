...
import splunktracing.constants
import splunktracing.recorder
...
def recorder(request):
    ...
    recorder = splunktracing.recorder.Recorder(runtime_args)
    yield recorder
...
def check_spans(converter, report):
    ...
    def create_test_recorder(self):
        """Returns a Splunk Recorder based on self.runtime_args.
        """
        return splunktracing.recorder.Recorder(**self.runtime_args)
    ...
    def dummy_basic_span(self, recorder, i):
        return BasicSpan(
            splunktracing.tracer._SplunkTracer(False, recorder, None),
